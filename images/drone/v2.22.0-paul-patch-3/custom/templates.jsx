// Полный файл src/pages/settings/templates/templates.jsx с удалённым MonacoEditor
...
const EditTemplateForm = ({ handleSubmit, handleCancel, editModalInitialState }) => {
  ...
  const handleAddTemplate = () => {
    handleSubmit({
      templateData: state.data,
      templateName: editModalInitialState.name,
    });
    handleCancel();
  };
  ...
  return (
    <Form className={cx('container', 'form')}>
      ...
      <FormSection className={cx('controls')}>
        <Button onClick={handleAddTemplate}>Update</Button>
        <Button onClick={handleCancel}>Cancel</Button>
      </FormSection>
    </Form>
  );
