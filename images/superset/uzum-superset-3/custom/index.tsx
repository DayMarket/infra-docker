...
const CONFIRM_OVERWRITE_MESSAGE = t(
  'You are importing one or more dashboards that already exist. Overwriting might cause you to lose some of your work.',
);
...
function DashboardList(props: DashboardListProps) {
  ...
  const [sshTunnelPrivateKeyPasswordFields, setSSHTunnelPrivateKeyPasswordFields] =
    useState<string[]>([]);
  ...
  const handleDashboardEdit = (edits: Dashboard) =>
    SupersetClient.get({ endpoint: `/api/v1/dashboard/${edits.id}` }).then(
      ({ json = {} }) => {
        setDashboards(
          dashboards.map(d => (d.id === json?.result?.id ? { ...d, ...json.result } : d)),
        );
      },
  ...
  const handleBulkDashboardExport = (toExport: Dashboard[]) => {
    const ids = toExport.map(({ id }) => id);
    handleResourceExport('dashboard', ids, () => setPreparingExport(false));
    setPreparingExport(true);
  };

  const handleBulkDashboardDelete = (toDelete: Dashboard[]) =>
    SupersetClient.delete({
      endpoint: `/api/v1/dashboard/?q=${rison.encode(toDelete.map(({ id }) => id))}`,
    }).then(
      ({ json = {} }) => {
        refreshData();
